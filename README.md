...
> ⚠️ **重要：** 开启15秒模式后，**提示词中不要写「15秒」或任何超过10秒的时长描述**，否则豆包会拒绝生成（提示「不支持超过10秒」），并提示生成10秒，如果继续生成的话就会变成10秒，导致插件失效。  
> **Important:** After enabling 15s mode, **do NOT include "15 seconds" or any duration over 10s in your prompt** — Doubao will reject it and suggest generating 10 seconds instead. If you proceed, the result will be 10 seconds and the plugin's 15s mode won't take effect.
